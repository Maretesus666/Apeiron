...
func _crear_boton(id: String, tipo: String) -> PanelContainer:
	...
	if not id in upgrades:
		...
		error_label.text = "Error: mejora '%s' no encontrada" % id
		error_label.add_theme_color_override("font_color", Color(1, 0.3, 0.3))
		error_panel.add_child(error_label)
		return error_panel
	
	var upgrade_data: Dictionary = upgrades[id]
	var max_level: int = upgrade_data.get("max_level", 999)
	var is_maxed: bool = upgrade_data["level"] >= max_level
	var cost: int = UpgradeManager.get_clicker_upgrade_cost(id) if tipo == "clicker" else UpgradeManager.get_ship_upgrade_cost(id)
	var pts: int = UpgradeManager.game_points if tipo == "clicker" else UpgradeManager.clicker_points
	...
	var name_color := Color(1.0, 0.85, 0.2) if is_maxed else (Color.WHITE if can_afford else Color(0.5, 0.5, 0.5))
	...
	var stats_color := Color(1.0, 0.85, 0.2) if is_maxed else (Color(0.75, 0.85, 0.95) if can_afford else Color(0.4, 0.4, 0.45))
	...
	if is_maxed:
		...
		max_label.add_theme_font_size_override("font_size", 28)
		max_label.add_theme_color_override("font_color", Color(1.0, 0.85, 0.2))
		max_label.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.8))
		max_label.add_theme_constant_override("outline_size", 2)
		max_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
		cost_vbox.add_child(max_label)
		
		var max_sublabel := Label.new()
		max_sublabel.text = "nivel %d/%d" % [upgrade_data["level"], max_level]
		max_sublabel.add_theme_font_override("font", FONT)
		max_sublabel.add_theme_font_size_override("font_size", 14)
		max_sublabel.add_theme_color_override("font_color", Color(0.8, 0.7, 0.3))
		max_sublabel.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
		cost_vbox.add_child(max_sublabel)
	else:
